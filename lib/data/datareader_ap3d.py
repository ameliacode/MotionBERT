...
class DataReaderH36M(object):
    ...
    def read_2d(self):
        trainset = np.array(self.dt_dataset["train"]["joint_2d"], dtype=np.float32)[
            :: self.sample_stride, :, :2
        ]  # [N, 17, 2]
        testset = np.array(self.dt_dataset["test"]["joint_2d"], dtype=np.float32)[
            :: self.sample_stride, :, :2
        ]  # [N, 17, 2]
        # map to [-1, 1]
        for idx, camera_name in enumerate(self.dt_dataset["train"]["camera_name"]):
            res_w, res_h = (
                self.dt_dataset["train"]["video_w"][idx],
                self.dt_dataset["train"]["video_h"][idx],
            )
            trainset[idx, :, :] = trainset[idx, :, :] / res_w * 2 - [1, res_h / res_w]
        for idx, camera_name in enumerate(self.dt_dataset["test"]["camera_name"]):
            res_w, res_h = (
                self.dt_dataset["test"]["video_w"][idx],
                self.dt_dataset["test"]["video_h"][idx],
            )
            testset[idx, :, :] = testset[idx, :, :] / res_w * 2 - [1, res_h / res_w]
        if self.read_confidence:
            if "confidence" in self.dt_dataset["train"].keys():
                train_confidence = self.dt_dataset["train"]["confidence"][
                    :: self.sample_stride
                ].astype(np.float32)
                test_confidence = self.dt_dataset["test"]["confidence"][
                    :: self.sample_stride
                ].astype(np.float32)
    ...
    def read_3d(self):
        train_labels = np.array(
            self.dt_dataset["train"]["joint3d_image"], dtype=np.float32
        )[
            :: self.sample_stride, :, :3
        ]  # [N, 17, 3]
        test_labels = np.array(
            self.dt_dataset["test"]["joint3d_image"], dtype=np.float32
        )[
            :: self.sample_stride, :, :3
        ]  # [N, 17, 3]
        # map to [-1, 1]
    ...
    def get_sliced_data(self):
        ...
        train_labels, test_labels = (
            train_labels[split_id_train],
            test_labels[split_id_test],
        )
        # pdb.set_trace()
    ...
    def denormalize(self, test_data, all_sequence=False):
        ...
        assert len(data) == len(
            test_hw
        ), f"Data n_clips is {len(data)} while test_hw size is {len(test_hw)}"
    ...
    def get_data_by_action(self, action_name, split="test"):
        """
        Get data filtered by specific action name

        Args:
            action_name (str): Name of the action to filter by
            split (str): 'train' or 'test' split

        Returns:
            tuple: (data, labels) for the specified action
        """
        if split == "test":
            # Get action list for test set
            actions = self.dt_dataset["test"]["action"]

            # Find indices where action matches
            action_indices = [
                i for i, action in enumerate(actions) if action == action_name
            ]
            ...
            # Get the data
            _, test_data = self.read_2d()
            _, test_labels = self.read_3d()

            # Filter by action indices
            filtered_data = test_data[action_indices]
            filtered_labels = test_labels[action_indices]

            return filtered_data, filtered_labels

        else:
            print("Action filtering only available for test split")
            return None, None

    def get_available_actions(self):
        """Get list of unique actions in the test set"""
        return list(set(self.dt_dataset["test"]["action"]))
    ...
    def preprocess(self, dataset):
        ...
        for i in range(len(dataset[key])):
            ...
            new_dataset[key]["joints_2.5d_image"].append(
                np.array(dict_i["joint_3d_image"]) / dict_i["ratio"]
            )
